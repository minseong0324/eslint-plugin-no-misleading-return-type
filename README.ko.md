...
> **📣 이 규칙이 [Biome](https://biomejs.dev/)에도 추가됐습니다 (Biome `v2.4.11`부터 사용 가능): [`noMisleadingReturnType`](https://biomejs.dev/linter/rules/no-misleading-return-type/)**
>
> Biome을 쓴다면 플러그인 설치 없이 바로 켜서 쓸 수 있으니 많이 써보시고 주변에도 알려주세요.
>
> Biome 쪽과 이 플러그인 모두 계속 유지보수하고 개선해 나갈 예정입니다. 쓰는 툴에 맞춰 편한 쪽을 고르시면 됩니다.

## 이 규칙이 필요한 이유

TypeScript는 실제 구현보다 **더 넓은(덜 정확한) 반환 타입 어노테이션**을 허용합니다. 이로 인해 의도적으로 만든 정확성이 조용히 사라집니다.
...
이 규칙은 어노테이션 타입이 추론 타입보다 넓은 경우를 감지하여, 구현이 제공하는 정확성을 보존하도록 도와줍니다.
...
함수의 명시적 반환 타입 어노테이션이 TypeScript의 추론 타입보다 **넓은** 경우를 보고합니다.
...
```ts
// 어노테이션 없음 — TypeScript가 정확한 타입을 추론
function getErrorMessages() {
  return {
    INVALID_TOKEN: 'Please log in again.',
    NETWORK_ERROR: 'Check your network connection.',
...
```ts
// as const 맵이 명시적 어노테이션으로 인해 넓어짐
function getErrorMessages(): Record<string, string> {
  return {
    INVALID_TOKEN: 'Please log in again.',
    NETWORK_ERROR: 'Check your network connection.',
...
function getStatus(loading: boolean): string {
  if (loading) return "loading";
  return "idle";                                 // 추론: "loading" | "idle", 어노테이션: string
}

// 비동기 여러 반환값
async function getStatus(x: boolean): Promise<string> {
  if (x) return "a";
  return "b";                                    // 추론: Promise<"a" | "b">, 어노테이션: Promise<string>
}
```
